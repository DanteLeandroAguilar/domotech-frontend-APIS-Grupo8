...
import { updateProductAmount } from '../../store/cartSlice';
...
export const CartItem = ({ item }) => {
  ...
  const handleIncrease = async () => {
    setLoading(true);
    try {
      await dispatch(updateProductAmount({ productId: item.productId, amount: item.amount + 1 })).unwrap();
    } catch (error) {
  ...
  const handleRemove = async () => {
    setLoading(true);
    try {
      await dispatch(updateProductAmount({ productId: item.productId, amount: 0 })).unwrap();
    } catch (error) {
