...
export const ProductForm = ({ product, onSubmit, onCancel }) => {
  ...
  useEffect(() => {
    if (!categoriesFromStore || categoriesFromStore.length === 0) dispatch(fetchCategories());
    // sync local categories state
  ...
  }, [product, dispatch, categoriesFromStore]);

  // loadCategories logic moved to useEffect which reads from the store
