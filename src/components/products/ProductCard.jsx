...
import { updateProductAmount, selectCartItems } from '../../store/cartSlice';
...
export const ProductCard = ({ product, updateCartCount }) => {
  ...
  const handleAddToCart = async (e) => {
    ...
    try {
      ...
      const existingItem = cartItems?.find((it) => it.productId === product.productId);
      const currentAmount = existingItem ? Number(existingItem.amount || 0) : 0;
      const desired = currentAmount + 1;
      const newAmount = desired;

      await dispatch(updateProductAmount({ productId: product.productId, amount: newAmount })).unwrap();
      if (updateCartCount) updateCartCount();
      console.log('Producto agregado al carrito (via Redux)');
    } catch (error) {
